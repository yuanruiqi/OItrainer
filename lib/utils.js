...
function generateName(province_id=-1,used = new Set()){
  ...
  let oierdb_province;
  if(province_id<0){
    const names=_globalNamePool;
    const name=generateNameByProvince(names,used);
    return name;
  }else {
    oierdb_province=OIERDB_PROVINCES.indexOf(PROVINCES[province_id]['name']);
